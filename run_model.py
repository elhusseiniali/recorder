...
import numpy as np
from run import test, solveone2
...
linenode = ['Statement_ter', 'BreakStatement_ter', 'ReturnStatement_ter',
            'ContinueStatement', 'ContinueStatement_ter',
            'LocalVariableDeclaration', 'condition', 'control',
            'BreakStatement', 'ContinueStatement', 'ReturnStatement',
            'parameters', 'StatementExpression', 'return_type']
...
def getSubroot(treeroot):
    ...
    currnode = treeroot
    while currnode:
        if currnode.name == 'MethodDeclaration' \
                or currnode.name == 'ConstructorDeclaration':
            mnode = currnode
            break
        currnode = currnode.father
    return lnode, mnode
...
def getNodeById(root, line):
    if root.position:
        if root.position.line == line and \
                root.name != 'IfStatement' and root.name != 'ForStatement':
            return root
    for x in root.child:
...
if True:
    ...
    data.append({'treeroot': treeroot, 'troot': troot,
                 'oldcode': oldcode, 'filepath': filepath, 'subroot': subroot,
                 'vardic': vardic, 'typedic': typedic, 'precode': precode,
                 'aftercode': aftercode,
                 'tree': troot.printTreeWithVar(troot, vardic),
                 'prob': troot.getTreeProb(troot),
                 'mode': 0, 'line': lineid, 'isa': False})
...
for p in ans:
    ...
    for l in code.splitlines():
        if l.strip() != "":
            ...
            continue
    if mode == 1 and len(precode.splitlines()) > 0 \
            and 'case' in precode.splitlines()[-1]:
